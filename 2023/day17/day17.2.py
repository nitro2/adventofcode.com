...
def mark_energized(data):
    ...
    init_list = []
    for i in range(h):
        init_list.append([((i,0),Dir.RIGHT)])
        init_list.append([((i,w-1),Dir.LEFT)])
    for i in range(w):
        init_list.append([((0,i),Dir.DOWN)])
        init_list.append([((h-1,i),Dir.UP)])
    print("Total cases: ", len(init_list))
    count = 0
    for init in init_list:
        count +=1
        res = 0
        energized = np.zeros(shape=(w, h))
        next_position_list = init
        for pos, dir in next_position_list:
            ...
            if validposition(pos, w, h):
                ...
                if symbol == '.':
                    ...
                    raise
        res =len(np.where(energized != 0)[0])
        result = max(res, result)
        if count %10 == 0:
            print("Temp result", result, "count", count, "time", (time.time() - start_time))
    return result
...
if __name__ == '__main__':
    assert solution("input16.sample.txt") == 51
    filename = sys.argv[1]
